...
这份手册写给第一次使用 天玄·千枢科学发现平台 的 macOS 用户。你不需要懂命令行，首次缺少模型凭证时会先进入初始配置；需要更换模型、API key、工作区或授权模式时，再进入配置页调整。

## 1. 安装和打开

1. Apple Silicon Mac 双击 `InternAgentS-0.1.0-arm64.dmg`；Intel Mac 双击 `InternAgentS-0.1.0-x64.dmg`。
2. 把 `天玄·千枢科学发现平台` 拖到 `Applications`（应用程序）文件夹。
3. 在「应用程序」里打开 `天玄·千枢科学发现平台`。

如果 macOS 提示“无法验证开发者”，可以在「应用程序」里按住 `Control` 点 `天玄·千枢科学发现平台`，选择「打开」。第一次打开后，以后通常就可以直接双击打开。
...
- API key 只保存在这台 Mac 的 天玄·千枢科学发现平台 设置里。
...
天玄·千枢科学发现平台 的操作范围主要在你设置的工作区里。新用户建议先使用 `自动授权` 熟悉流程；如果你希望写文件前都确认，可以在配置页选择更严格的授权模式。
