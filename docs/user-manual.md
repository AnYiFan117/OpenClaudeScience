# 天玄·千枢科学发现平台 用户手册（新手版）

这份手册写给第一次使用 天玄·千枢科学发现平台 的用户。你可以把它当作一份“照着点”的操作说明：先进入工作台，再放入资料、发起任务、查看结果，最后根据需要配置模型、技能和更新。

> 说明：截图来自当前本地开发版界面。桌面正式版和不同部署环境的按钮位置可能略有差异，但核心流程一致。

## 目录

1. [天玄·千枢科学发现平台 是什么](#internagents-是什么)
2. [进入 天玄·千枢科学发现平台](#进入-internagents)
...
天玄·千枢科学发现平台 是一个面向科研、代码和技术探索的智能体工作台。它不仅能聊天，还能围绕一个工作区读取文件、理解文档、拆解任务、生成内容、调用工具，并在需要时让用户审批关键操作。
...
- 工作区：天玄·千枢科学发现平台 能看到和操作的文件夹。
- 会话：你和 天玄·千枢科学发现平台 围绕某个问题的一段对话记录。
...
如果第一次打开时 macOS 提示无法验证开发者，可以在“应用程序”中按住 `Control` 点 `天玄·千枢科学发现平台`，再选择“打开”。
...
工作区是 天玄·千枢科学发现平台 的“活动范围”。新手可以这样理解：

```text
你把资料放进工作区，天玄·千枢科学发现平台 才能围绕这些资料工作。
```
...
`/goal` 适合较长、需要多步骤推进的任务。它会把目标记录在当前会话里，让 天玄·千枢科学发现平台 按目标继续工作。
...
天玄·千枢科学发现平台 可能会调用工具读取文件、搜索文本、写文件、编辑文件或执行命令。是否需要你确认，取决于配置页中的授权模式。
...
`本机工作区路径` 决定 local 模式下 天玄·千枢科学发现平台 能看到哪个文件夹。
...
技能在配置页中的 `技能` 卡片里管理。技能可以给 天玄·千枢科学发现平台 增加某类任务的专长。
...
- `服务器地址`：LangGraph / 天玄·千枢科学发现平台 服务地址，例如 `http://127.0.0.1:2024`。
...
页面会说明 天玄·千枢科学发现平台 的定位：它是面向科研与技术探索的智能体工作台。
